...
def plot_overhead(df, ax, is_sng, title):
    ...
    df_mean = df.groupby(['inter', 'work', 'msg_size', 'offload', 'nthreads']).mean().reset_index()
    ...
    if is_sng:
        ncol=1
        legend_labels = [f'1 MPI proc per socket' + str_native, f'1 MPI proc per socket' + str_offload]
    else:
        ncol=2
        legend_labels = [f"{'1 MPI proc per socket' if type[0] else '1 MPI proc per NUMA domain'} {str_offload if type[1] else str_native}" for (type) in df_mean.groupby(['inter', 'offload']).groups]
...
if __name__ == '__main__':
    ...
    plot_overhead(hwak_df_compute, axs[0], False, subtitle_hawk)

    # ----------------------------------
    # SNG data and plot
    # ----------------------------------
    sng_data_dir = './sng/result'
    csv_files_compute = [#sng_data_dir + '/intra_native_compute_1node_2procs.csv',
                         #sng_data_dir + '/intra_offload_compute_1node_2procs.csv',
                         sng_data_dir + '/inter_native_compute_2nodes_4procs.csv', 
                         sng_data_dir + '/inter_offload_compute_2nodes_4procs.csv']
    # csv_files_mem = [sng_data_dir + '/inter_native_mem_2nodes_4procs.csv', 
    #                  sng_data_dir + '/inter_offload_mem_2nodes_4procs.csv']
    ...
    plot_overhead(df_compute_intra_inter, axs[1], True, subtitle_sng)
    
    # fig.suptitle(f'Wait Time on SNG: Intra- and Inter-Node Communication')
    axs[0].set_ylabel('Wait Time [ns]')
    
    plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=0.5)
    plt.subplots_adjust(top=0.90)
    plt.savefig(f'./overhead_microbench_hawk_sng.pdf')
    print("Saved figure to ./overhead_microbench_hawk_sng.pdf")
