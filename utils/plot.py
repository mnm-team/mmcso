...
def plot_overlap(df, col, fig, ax, mpi):
    ...
    ax[(0,0)].legend(legend_labels,
                                 ...
                                 frameon=True,
                                 framealpha=0.8,
                                 title="Threads / Offload"
                                 )

if __name__ == '__main__':

    fig, ax = plt.subplots(3, 2, figsize=(16, 9), sharex=True, sharey=True)
    ...
    plot_overlap(df, 0, fig, ax, 'IntelMPI')
    ...
    plot_overlap(df, 1, fig, ax, 'OpenMPI')
    # plot_overhead(df, 0)
    ...
    plt.savefig(f'commoff_overlap.pdf')
